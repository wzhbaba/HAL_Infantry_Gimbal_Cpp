...
/* Private variables ---------------------------------------------------------*/
/* External variables --------------------------------------------------------*/
Uart_RX_Message UART_Instance[4];
...
void UserUARTReceiveIT(UART_HandleTypeDef *huart, uint32_t length, UART_CallBack Function)
{
    if (huart->Instance == USART1) {
        ...
        UART_Instance[3].huart = huart;
        UART_Instance[3].rx_buffer_size = length;
        UART_Instance[3].Function = Function;
        HAL_UARTEx_ReceiveToIdle_DMA(UART_Instance[3].huart, UART_Instance[3].rx_buffer, UART_Instance[3].rx_buffer_size);
        __HAL_DMA_DISABLE_IT(UART_Instance[3].huart->hdmarx, DMA_IT_HT);
    }
}
...
void UserUARTSendData(UART_HandleTypeDef *huart, uint8_t *send_buf, uint16_t send_size, UART_TRANSMIT_MODE MODE)
{
    if (MODE == UART_TRAMSMIT_BLOCKING) {
        HAL_UART_Transmit(huart, send_buf, send_size, 100);
